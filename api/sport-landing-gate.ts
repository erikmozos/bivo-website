...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  const expectedUser = (
    process.env.SPORT_LANDING_GATE_USER?.trim() ||
    process.env.BADMINTON_GATE_USER?.trim() ||
    DEFAULT_USER
  ).toLowerCase();
  const expectedPassword =
    process.env.SPORT_LANDING_GATE_PASSWORD ||
    process.env.BADMINTON_GATE_PASSWORD ||
    DEFAULT_PASSWORD;
